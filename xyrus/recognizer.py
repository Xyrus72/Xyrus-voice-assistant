...
# Same for a call from across the room (distance bench, Sep 15 2026, peak 0.018: Vosk missed the name, Whisper
# heard the request right but wrote "gyrus, what time is it?" / "Dyrus, play Shape of You..."; live: "Tyrus.")
CALL_NAMES = frozenset({"sirens", "siren", "gyrus", "dyrus", "tyrus"})
...
CONFIRM_LONG_S = 3.0
CONFIRM_PEAK = 0.08
# "faint" / "name lost" are WEAK doubts: every call from across the room has them. Whisper still hears such a wake
# (its command text is better), but its "no" only drops it when Whisper itself was sure - the lost calls read
# "Tyrus." (-0.90), "Hi, Rus." (-1.00), "Hi, Ross." (-0.89); a confidently heard other word ("Serious." -0.05..-0.21)
# still drops it (Sep 15 2026)
WEAK_DOUBTS = frozenset({"faint", "name lost"})
KEEP_WHISPER_LP = -0.3
# "Hi, I'm Xyrus." / "Hey, it's Xyrus": up to two of these may come before the name
NAME_LEADERS = frozenset({"i'm", "im", "it's", "its", "yo"})
...
def _strong_doubt(why: str) -> bool:
    ...
    or low-confidence decode) - not only "faint" / "name lost", which every call from a distance has."""
...
class Recognizer:
    ...
    def _whisper_name_check(self, pcm: bytes, handed: bool = False) -> tuple[bool, list[str], str, float | None] | None:
        ...
        i = 0
        while i < 2 and len(words) > i + 1 and (words[i] in WAKE_LEADERS or words[i] in NAME_LEADERS):
            i += 1
