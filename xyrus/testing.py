...
_ACTION_DEFAULTS: dict[str, Any] = {
    ...
    # songs
    "browser_playing": False, "ensure_playing": "autoplay", "media_sounding": None,
    # info
...
class FakeActions:
    ...
    def _default_media_sounding(self):
        """Unless a test sets media_sounding: the same answer as browser_playing (playing = sounding)."""
        r = self.returns.get("browser_playing", _ACTION_DEFAULTS["browser_playing"])
        return r() if callable(r) else r

    def _default_find_song(self, query, alternates=(), bn_text=None):
        return (query, "https://www.youtube.com/watch?v=xxxxxxxxxxx")   # hints recorded in calls, not used
