...
DEFAULTS: dict = {
    ...
    # 0.01: calls from across the room peak ~0.018-0.025; 0.028 lost 5/16 at 0.025 and all at 0.018. Room noise,
    # sound-alikes and name-less commands still woke nothing at 0.006 (distance bench, Sep 15 2026)
    "min_speech_peak": 0.01,
    # hearing rebuild (Sep 15 2026). hearing_model: auto|large-v3|large-v3-int8|turbo|small (auto = the best
    # installed on the GPU); vad: silero|energy (capture endpointing); wake_confirm: Whisper confirms doubtful
    # Vosk wakes; arm_timeout_s: 0 = wait for the command with no limit (the user's choice), else seconds;
    # logprob_enforce: drop low-confidence Whisper text (off = log only); bangla_model: auto|off (the Bengali
    # fine-tune in models/whisper-large-v3-bn when present)
    "hearing_model": "auto",
    "vad": "silero",
    "wake_confirm": True,
    "arm_timeout_s": 0.0,
    "logprob_enforce": False,
    "bangla_model": "auto",
