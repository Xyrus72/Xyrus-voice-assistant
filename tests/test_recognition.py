...
# ============================================================================ hearing rebuild: Whisper is the wake authority
class WhisperWakeLive(unittest.TestCase):
    """Real Vosk (wake grammar, command re-decode) + a canned Whisper through Recognizer._process: doubtful
    Vosk wakes are confirmed, a command said with the name goes to engine.on_wake_with_command, re-decodes
    get the speech span only, and while a song plays idle utterances are cut at 3 s."""

    def make(self, hearing=None, spec=ListenSpec("wake"), hooks=True):
        from tests.test_free_decoder import Hooks
        got = []
        rec = new_recognizer(FakeCapture(), lambda: spec, got)
    ...
    @staticmethod
    def wakes(got):
        return [tr for tr in got if WAKE & set(tr.text.split())]

    def canned(self, en, lp=-0.2):
        from tests.test_free_decoder import CannedHearing
        return CannedHearing(en=en, lp=lp)

    def test_faint_wake_is_confirmed_and_answered_once(self):
        h = self.canned("Xyrus.")
        rec, got = self.make(h)
        self.play(rec, W.silence(1) + W.faint(W.synth("xyrus"), 0.05) + W.silence(2.5))
        self.assertEqual(len(self.wakes(got)), 1, (got, dict(rec.stats)))
        self.assertGreaterEqual(len(h.pcms), 1, "Whisper decided")

    def test_faint_clear_name_is_kept_when_whisper_disagrees(self):
        """Sep 15: from a distance Vosk heard the real name, Whisper misspelled it ("Tyrus.") and calls were lost -
        the doubts were only "faint" / "name lost", so an unsure Whisper "no" never drops the wake."""
        h = self.canned("Tyrus.", lp=-0.90)
        rec, got = self.make(h)
        self.play(rec, W.silence(1) + W.faint(W.synth("xyrus"), 0.05) + W.silence(2.5))
        self.assertEqual(len(self.wakes(got)) + len(rec.engine.woke), 1, (got, dict(rec.stats)))

    def test_faint_speech_confident_whisper_rejects_never_wakes(self):
        h = self.canned("Volume up.", lp=-0.1)
        rec, got = self.make(h)
        self.play(rec, W.silence(1) + W.faint(W.synth("xyrus"), 0.05) + W.silence(2.5))
        self.assertEqual(self.wakes(got), [], dict(rec.stats))
        self.assertEqual(rec.engine.woke, [])

    def test_unk_padded_wake_is_confirmed(self):
        pcm = W.silence(1) + W.synth("xyrus purple elephants") + W.silence(2.5)
        rec, got = self.make(self.canned("Purple elephants."))
        self.play(rec, pcm)
        self.assertEqual((self.wakes(got), rec.engine.woke), ([], []), dict(rec.stats))
        rec, got = self.make(self.canned("Xyrus, purple elephants."))
        self.play(rec, pcm)
        self.assertEqual([r for r, _ in rec.engine.woke], ["purple elephants"], (got, dict(rec.stats)))
        self.assertEqual(self.wakes(got), [])

    def test_one_breath_command_reaches_the_engine_whole(self):
        pcm = W.silence(1) + W.faint(W.synth("xyrus take a screenshot"), 0.05) + W.silence(2.5)
        rec, got = self.make(self.canned("Xyrus, take a screenshot."))
        self.play(rec, pcm)
        self.assertEqual([r for r, _ in rec.engine.woke], ["take a screenshot"], (got, dict(rec.stats)))
        rec, got = self.make(self.canned("Xyrus, take a screenshot."), hooks=False)      # no hook: one transcript
        self.play(rec, pcm)
        self.assertEqual(len(self.wakes(got)), 1, got)
        self.assertIn("screenshot", self.wakes(got)[0].text)

    def test_a_clean_loud_wake_costs_no_whisper(self):
        h = self.canned("never used")
        rec, got = self.make(h)
        self.play(rec, W.silence(1) + W.synth("xyrus") + W.silence(2.5))
        self.assertEqual(len(self.wakes(got)), 1, got)
        self.assertEqual(h.pcms, [])

    def test_redecodes_get_the_speech_span_only(self):
        h = self.canned("Xyrus.")
        rec, got = self.make(h)
        self.play(rec, W.noise(12, 2500, seed=11) + W.synth("xyrus purple elephants") + W.silence(2.5))
        self.assertTrue(h.pcms or rec._recent, dict(rec.stats))
        self.assertTrue(all(len(p) / 32000 <= 6.6 for p in h.pcms), [len(p) / 32000 for p in h.pcms])
        self.assertTrue(all(len(p) / 32000 <= 6.6 for _, _, p in rec._recent))

    def _longest_utt(self, rec, pcm):
        longest, t = 0, 100.0
        for c in W.chunks(pcm):
            t += 0.25
            rec._process(c, t)
    ...
    def test_playback_cuts_idle_utterances_at_3_s(self):
        rec, _ = self.make(self.canned(""), spec=ListenSpec("wake", playback=True))
        self.assertLessEqual(self._longest_utt(rec, W.noise(10, 3000, seed=7)), 3.0 + 0.25)
        self.assertGreaterEqual(rec.stats["playback_flushes"], 2)

    def test_without_playback_idle_timing_is_untouched(self):
        rec, _ = self.make(self.canned(""), spec=ListenSpec("wake"))
        self.assertGreater(self._longest_utt(rec, W.noise(10, 3000, seed=7)), 3.5)
        self.assertEqual(rec.stats["playback_flushes"], 0)
