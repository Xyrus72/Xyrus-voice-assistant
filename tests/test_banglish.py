...
class CarrierWords(unittest.TestCase):
    ...
    def test_hinted_bare_title_and_noise(self):
        self.assertEqual(B.song_request("ঈশ্বর", hinted=True), "ঈশ্বর")
        self.assertIsNone(B.song_request("ঈশ্বর"))
        self.assertIsNone(B.song_request("াাাাাাাা বাজাও"))                 # no consonant
        self.assertIsNone(B.song_request("কককক বাজাও"))                    # 4x the same character
        self.assertIsNone(B.song_request("play believer", hinted=True))     # no Bengali at all

    def test_name_is_eaten_only_exactly_and_first(self):
        self.assertFalse(B._is_name("সিরিয়াস"))                            # used to fuzzy-match "serious"
        self.assertEqual(B.song_request("সাইরাস সিরিয়াস বাজাও"), "সিরিয়াস")
        self.assertEqual(B.song_request("প্লে সিরিয়াস"), "সিরিয়াস")
        self.assertEqual(B.song_request("সিরিয়াস বাজাও"), "সিরিয়াস")


class BnAcceptable(unittest.TestCase):
    def test_rejects(self):
        self.assertFalse(B.bn_acceptable("কে স্র স্র স্র স্র", -0.2, -0.3, 1))    # 5 tokens > 3x the English 1
        self.assertFalse(B.bn_acceptable("স্র স্র স্র স্র স্র স্র স্র স্র স্র স্র স্র স্র", -0.2, -0.3, None))   # compresses
        self.assertFalse(B.bn_acceptable("PLAY ISHVAR BY VIKINGS", -0.1, -0.3, 4))                  # Latin only
        self.assertFalse(B.bn_acceptable("াাাাাাাা", -0.1, -0.3, 1))                               # no consonant
        self.assertFalse(B.bn_acceptable("ঈশ্বর", -0.6, -0.3, 3))                                 # scores too badly
        self.assertFalse(B.bn_acceptable("", None, None, None))

    def test_accepts(self):
        self.assertTrue(B.bn_acceptable("ঈশ্বর", -0.4, -0.3, 3))
        self.assertTrue(B.bn_acceptable("ঈশ্বর", -2.0, None, 3))                  # no English score: no constraint
        self.assertTrue(B.bn_acceptable("আমার ভিনদেশী তারা", -0.3, -0.3, 1))
