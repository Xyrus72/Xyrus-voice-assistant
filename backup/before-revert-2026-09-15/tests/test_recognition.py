...
class WhisperWakeLive(unittest.TestCase):
    ...
    def test_faint_clear_name_is_kept_when_whisper_disagrees(self):
        """Sep 15: Vosk heard the real name, Whisper misspelled it ("Hi, Ross.") and real calls were lost - the
        doubts were only "faint" / "name lost", so Whisper's "no" never drops the wake."""
        h = self.canned("Hi, Ross.", lp=-0.89)                 # the real reading was unsure (-0.89)
        rec, got = self.make(h)
        self.play(rec, W.silence(1) + W.faint(W.synth("xyrus"), 0.05) + W.silence(2.5))
