...
class WakeDecisions(unittest.TestCase):
    ...
    def test_faint_clear_name_is_heard_by_whisper_but_never_dropped(self):
        """Sep 15: only "faint" / "name lost" -> Whisper hears it (better command text), but its "no" keeps the
        wake: it misspelled the real name ("Hi, Ross.") and calls were lost."""
        rec, _, h = self.rec("Hi, Ross.", lp=-0.89)            # the real reading was unsure (-0.89)
