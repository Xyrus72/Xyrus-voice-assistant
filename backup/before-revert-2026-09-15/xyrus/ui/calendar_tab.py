"""Calendar tab: month grid, day agenda (to-dos with tick boxes, events, notes), quick-add and the
upcoming list. Lifted from v1 D:\\arc\\calendar_tab.py (D14) and adapted to App + ui/theme.py.

Everything here runs on the Tk main thread. The data lives in the CalendarStore (app.store), which the
engine thread also changes; poll() (called from XyrusWindow._poll every 150 ms) compares store.version
and re-renders only what is stale, plus once a minute. customtkinter widgets are slow to create, so the
42 day cells are built once and re-configured, and agenda / upcoming rows come from pools that only grow.

Changes vs v1: constructor takes `app`; colours from theme; `dates` instead of `when`; keyboard
navigation (arrows ±1 day, Up/Down ±1 week, PgUp/PgDn ±1 month, Home = today); upcoming = next 14 days;
✎ opens EventDialog; event ✕ asks inline "Delete? Yes"; a leading "todo"/"to do"/"task" in quick-add
makes a to-do; live preview through dates.parse_typed when present.
"""
...
except ImportError:                 # SPEC-AMBIGUITY: bridge until T2's dates.py (= v1 when.py) lands
    import when as W                # type: ignore[no-redef]

from xyrus.ui.theme import (ACCENT, ACCENT_HOVER, ACCENT_SOFT, BG, CARD, CHECK_BORDER, DIM, FAINT, GREEN,
                            GREEN_HOVER, LINE, LINE_HOVER, MENU, MUTED, NOTE, ON_ACCENT, ON_ACCENT_SOFT, RANGE, RED,
                            RED_HOVER, SEG, SEL_RING, STAR, SURFACE, SURFACE_HOVER, TEXT, TITLE, WEEKEND, c)

from xyrus.ui.theme import mode as theme_mode  # noqa: E402

log = logging.getLogger("xyrus.ui.calendar")
AMBER = NOTE                        # v1 name for the notes colour
...
UP_DAYS = 14                 # §4.15: upcoming = next 14 days (v1 showed 7)
CONFIRM_MS = 5000            # inline "Delete? Yes" gives up after this
WARM_ROWS = 24               # agenda rows pre-built in idle time after start
RELATIVE_TIME = re.compile(r"\bin\s+(?:\w+\s+)?(?:minutes?|mins?|hours?|hrs?)\b|\bin\s+half\s+an?\s+hour\b", re.I)
TODO_PREFIX = re.compile(r"^\s*(?:to\s*-?\s*do|task)\b\s*[:,-]?\s*", re.I)
...
def parse_typed(text: str, now: dt.datetime):
    """dates.parse_typed (parse_when + parsedatetime fallback) when T2 ships it, else parse_when."""
    fn = getattr(W, "parse_typed", None)
    if fn is not None:
        try:
            return fn(text, now)
        except Exception:                    # e.g. parsedatetime missing - never break typing
            log.debug("parse_typed failed for %r", text, exc_info=True)
    return W.parse_when(text, now)
...
class _Cell:
    ...
    def __init__(self, tab: "CalendarTab", parent):
        ...
        self.cv.bind("<Double-Button-1>", lambda e: self.date and tab.new_item_dialog(self.date), add="+")
    ...
    def show(self, d, in_month, is_today, selected, in_range, counts):
        ...
        border = (SEL_RING if selected else ACCENT) if is_today else fg
    ...
    def _draw_text(self):
        ...
        num_id = cv.create_text(pad, round(4 * s), text=str(d.day), anchor="nw", fill=c(num),
                                font=t.cfont(12, "bold" if (is_today or selected) else "normal"), tags="cal")
        nb = cv.bbox(num_id)
        parts = []
        if counts:
            dim = not in_month and not selected
            on_sel = selected and theme_mode() == "light"   # green/amber dots vanish on the light accent fill
            ev = counts.get("event", 0)
            open_ = counts.get("task_open", 0)
            done = counts.get("task", 0) - open_
            nt = counts.get("note", 0)
            if ev:
                parts.append(("●", ON_ACCENT if selected else (DIM if dim else ACCENT_SOFT), ev))
            if open_:
                parts.append(("●", ON_ACCENT_SOFT if on_sel else DIM if dim else GREEN, open_))
            elif done:
                parts.append(("✓", ON_ACCENT_SOFT if selected else DIM if dim else MUTED, done))
            if nt:
                parts.append(("●", ON_ACCENT_SOFT if on_sel else DIM if dim else AMBER, nt))
        if parts:
                ...
                if compact:
                    ...
                    for glyph, color, n in reversed(parts):
                        if with_counts and n > 1:
                            cv.create_text(x, y + 1, text=str(n), anchor="se", fill=c(color),
                                           font=t.cfont(9, "bold"), tags=("cal", "dots"))
                            x = cv.bbox("dots")[0] - 1
                        cv.create_text(x, y, text=glyph, anchor="se", fill=c(color),
                                       font=t.cfont(8), tags=("cal", "dots"))
                        x = cv.bbox("dots")[0] - round(2 * s)
                else:
                    x = pad - round(1 * s)
                    for glyph, color, n in parts:
                        cv.create_text(x, y, text=glyph, anchor="sw", fill=c(color),
                                       font=t.cfont(8), tags=("cal", "dots"))
                        x = cv.bbox("dots")[2] + 1
                        if with_counts and n > 1:
                            cv.create_text(x, y + 1, text=str(n), anchor="sw", fill=c(color),
                                           font=t.cfont(9, "bold"), tags=("cal", "dots"))
                            x = cv.bbox("dots")[2]
                        x += round(3 * s)
                b = cv.bbox("dots")
...
class _Row:
    ...
    def __init__(self, tab: "CalendarTab", parent):
        ...
        self.check = ctk.CTkCheckBox(f, text="", width=22, checkbox_width=18, checkbox_height=18,
                                     corner_radius=5, border_width=2, fg_color=GREEN, hover_color=GREEN_HOVER,
                                     border_color=CHECK_BORDER, checkmark_color=BG,
        ...
        self.confirm = None                  # inline "Delete? Yes", built on first use (rows are pooled)
        self.confirm_yes = None
        # plain tk label (rows are pooled and a CTkButton costs ~10x more to create); looks like the ✕ button
        self.edit = tk.Label(f, text="✎", bg=c(BG), fg=c(FAINT), font=tab.f_small, width=2, cursor="hand2")
        self.edit.bind("<Button-1>", lambda e: tab.edit_dialog(self))
        self.edit.bind("<Enter>", lambda e: self.edit.configure(bg=c(LINE), fg=c(TEXT)))
        self.edit.bind("<Leave>", lambda e: self.edit.configure(bg=c(BG), fg=c(FAINT)))
        self.delete = ctk.CTkButton(f, text="✕", width=24, height=24, corner_radius=6, fg_color="transparent",
                                    ...
                                    command=lambda: tab._delete_clicked(self))
        ...
        self.edit.grid(row=0, column=4, padx=(2, 0))
        self.delete.grid(row=0, column=5, padx=(2, 4))
        self.title.bind("<Double-Button-1>", lambda e: tab._start_edit(self))
        self.lead.bind("<Button-1>", lambda e: self.kind == "note" and tab._start_edit(self))
        self._shape = None
        self._wrapped = None
    ...
    def _ensure_confirm(self):
        if self.confirm is None:
            tab = self.tab
            self.confirm = ctk.CTkFrame(self.frame, fg_color="transparent")
            ctk.CTkLabel(self.confirm, text="Delete?", font=tab.f_small, text_color=RED).pack(side="left", padx=(0, 6))
            self.confirm_yes = ctk.CTkButton(self.confirm, text="Yes", width=40, height=22, corner_radius=6,
                                             fg_color=RED, hover_color=RED_HOVER, text_color=ON_ACCENT,
                                             font=tab.f_small, command=lambda: tab._delete(self))
            self.confirm_yes.pack(side="left")

    def start_confirm(self):
        self._ensure_confirm()
        self.confirming = True
        self.meta.grid_remove()
        self.edit.grid_remove()
        self.confirm.grid(row=0, column=3, padx=(6, 2), sticky="e")
        self._confirm_job = self.frame.after(CONFIRM_MS, self.end_confirm)

    def end_confirm(self):
        if self._confirm_job:
            try:
                self.frame.after_cancel(self._confirm_job)
            except Exception:
                pass
            self._confirm_job = None
...
class CalendarTab(ctk.CTkFrame):
    """CalendarTab(master, app): uses app.store (None -> a disabled notice), app.clock, app.engine."""
    ...
    def __init__(self, master, app):
        ...
        self._now = clock.now if clock is not None else dt.datetime.now
        self.scale = ctk.ScalingTracker.get_widget_scaling(self)
        self.dialog = None                    # the open EventDialog, if any
        ...
        self._bind_keys()
        self._flush()
        self.after(1500, self._warm_rows)
    ...
    # ------------------------------------------------------------ building -- #
    def _warm_rows(self):
        """Grow the pooled agenda rows a few at a time in idle moments, so opening a busy day later only
        re-configures widgets instead of creating them (the one slow part of a day switch)."""
        if self.store is None or len(self.rows) >= WARM_ROWS:
            return
        for _ in range(3):
            self._row(len(self.rows))
        self.after(60, self._warm_rows)

    def on_theme(self):
        """Light/Dark switched: canvas text (day numbers, dots) and the plain-tk ✎ labels re-resolve; CTk
        widgets recolour themselves."""
        if self.store is None:
            return
        for cell in self.cells:
            ...
            cell.colors = None
        for r in self.rows:
            r.edit.configure(bg=c(BG), fg=c(FAINT))
        self._dirty |= {"grid", "day", "upcoming"}
        self._flush()
    ...
    def _build_unavailable(self):
        ...
        ctk.CTkLabel(box, text="Calendar unavailable", font=ctk.CTkFont(size=17, weight="bold"),
                     text_color=TITLE).place(relx=0.5, rely=0.45, anchor="center")
    ...
    def _build_month(self, left):
        ...
        self.month_lbl = ctk.CTkLabel(head, text="", width=138, font=self.f_month, text_color=TITLE)
        self.month_lbl.pack(side="left", padx=2)
        ctk.CTkButton(head, text="›", command=lambda: self.shift_month(1), **nav).pack(side="left")
        ctk.CTkButton(head, text="Today", width=62, height=26, corner_radius=8, fg_color=LINE,
                      hover_color=LINE_HOVER, text_color=TEXT, font=self.f_small,
                      command=self.go_today).pack(side="right")
        ...
        for c, name in enumerate(("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")):
            ...
            ctk.CTkLabel(wk, text=name, height=18, font=self.f_tiny,
                         text_color=MUTED if c < 5 else WEEKEND).grid(row=0, column=c)
    ...
    def _build_upcoming(self, left):
        ...
        ctk.CTkLabel(head, text=f"NEXT {UP_DAYS} DAYS", height=20, font=self.f_section,
    ...
    def _build_day(self, right):
        ...
        self.day_title = ctk.CTkLabel(bar, text="", font=self.f_day, text_color=TITLE, height=30)
        self.day_title.pack(side="left")
        self.day_tag = ctk.CTkLabel(bar, text="", height=22, corner_radius=8, fg_color=ACCENT,
                                    text_color=ON_ACCENT, font=ctk.CTkFont(size=11, weight="bold"))
        self.new_btn = ctk.CTkButton(bar, text="+", width=26, height=24, corner_radius=8, fg_color=LINE,
                                     hover_color=ACCENT_HOVER, text_color=TEXT, font=ctk.CTkFont(size=15),
                                     command=lambda: self.new_item_dialog(self.selected))
        self.new_btn.pack(side="right")
        self.day_sub = ctk.CTkLabel(bar, text="", height=22, font=self.f_small, text_color=MUTED)
        self.day_sub.pack(side="right", padx=(0, 8))
        self.day_bar = bar
        bar.bind("<Configure>", lambda e: self._fit_bar(), add="+")

        self.list = ctk.CTkScrollableFrame(right, fg_color="transparent", height=80,
                                           scrollbar_button_color=LINE, scrollbar_button_hover_color=LINE_HOVER)
        ...
        ctk.CTkLabel(self.empty, text="✦", font=ctk.CTkFont(size=26), text_color=STAR).pack(pady=(24, 0))
    ...
    def _build_quick_add(self, right):
        ...
        self.kind_seg = ctk.CTkSegmentedButton(top, values=list(KINDS), height=26, font=self.f_small,
                                               command=self._kind_changed, **SEG)
        ...
        self.rem_menu = ctk.CTkOptionMenu(top, values=[lab for lab, _ in REMINDERS], width=132, height=26,
                                          ...
                                          dynamic_resizing=False, command=self._rem_picked, **MENU)
        ...
        self.add_btn = ctk.CTkButton(mid, text="Add", width=60, height=32, corner_radius=8, fg_color=ACCENT,
                                     ...
                                     font=ctk.CTkFont(size=13, weight="bold"), command=self.quick_add)
        self.add_btn.pack(side="right", padx=(6, 0))
        self.entry = ctk.CTkEntry(mid, height=32, corner_radius=8, fg_color=BG, border_color=LINE,
                                  text_color=TEXT, font=self.f_row, placeholder_text=PLACEHOLDERS["To-do"])
        ...
        self.undo_btn = ctk.CTkButton(bottom, text="Undo", width=44, height=18, corner_radius=6, fg_color=LINE,
                                      hover_color=LINE_HOVER, text_color=TEXT, font=self.f_tiny,
                                      command=self.undo_delete)

    # ------------------------------------------------------------ keyboard -- #
    def _bind_keys(self):
        top = self.winfo_toplevel()
        keys = {"<Left>": lambda: self.step_day(-1), "<Right>": lambda: self.step_day(1),
                "<Up>": lambda: self.step_day(-7), "<Down>": lambda: self.step_day(7),
                "<Prior>": lambda: self.select(add_months(self.selected, -1)),
                "<Next>": lambda: self.select(add_months(self.selected, 1)),
                "<Home>": self.go_today}
        for seq, fn in keys.items():
            top.bind(seq, lambda e, fn=fn: self._on_key(e, fn), add="+")

    def _on_key(self, event, fn):
        """Arrow / PgUp / PgDn / Home move the selection - only while this tab is showing and the
        keyboard focus is not in a text field."""
        try:
            if not self.winfo_ismapped() or self.dialog is not None:
                return None
            cls = event.widget.winfo_class() if hasattr(event.widget, "winfo_class") else ""
            if cls in ("Entry", "Text", "TEntry", "Spinbox", "Listbox", "TCombobox"):
                return None
        except Exception:
            return None
        fn()
    ...
    # ------------------------------------------------------------ navigation -- #
    def step_day(self, days):
        self.select(self.selected + dt.timedelta(days=days))
    ...
    def _render_day(self):
        ...
        if tag:
            self.day_tag.configure(text=f"  {tag}  ", fg_color=ACCENT if tag == "Today" else LINE,
                                   text_color=ON_ACCENT if tag == "Today" else TEXT)
    ...
    def _fit_bar(self):
        need = (self.day_title.winfo_reqwidth() + self.day_sub.winfo_reqwidth()
                + self.new_btn.winfo_reqwidth() + 24)
    ...
    def upcoming_entries(self):
        ...
        end = today + dt.timedelta(days=UP_DAYS - 1)
        return [(d, it) for d, it in self.store.items_between(today, end, include_done=False)
    ...
    def _render_upcoming(self):
        ...
        for d, it in shown:
            delta = (d - today).days
            day = "Today" if delta == 0 else "Tmrw" if delta == 1 else f"{d:%a}" if delta < 7 else f"{d.day} {d:%b}"
        ...
        if not items and fit:
            rows.append(("", "", MUTED, f"Nothing in the next {UP_DAYS} days", None))
    ...
    def _delete_clicked(self, row: _Row):
        """✕: to-dos and notes go at once (Undo offered); events ask inline "Delete? Yes" first."""
        if row.kind == "event" and not row.confirming:
    ...
    # ---------------------------------------------------------- EventDialog -- #
    def _default_reminder(self):
        try:
            from xyrus.ui import cfg_get
            return cfg_get(self.app.config, "calendar.default_reminder_min", 10)
        except Exception:
            return 10

    def _open_dialog(self, **kw):
        from xyrus.ui.event_dialog import EventDialog
        ...
        def done(item):
            ...
            self._dirty |= {"grid", "day", "upcoming"}
            if item:
                self.select(item["date"])
                self._flash("Saved.", GREEN)
            else:
                self._flush()
                self._flash("Deleted.", MUTED)

        self.dialog = EventDialog(self, self.store, default_reminder=self._default_reminder(), on_done=done, **kw)
        self.dialog.bind("<Destroy>", lambda e: e.widget is self.dialog and setattr(self, "dialog", None), add="+")
        return self.dialog

    def edit_dialog(self, row: _Row):
        """✎ on a to-do / event row."""
        if row.obj and row.kind in ("task", "event"):
            return self._open_dialog(item=row.obj)
        return None

    def new_item_dialog(self, day=None):
        """+ in the day bar (or double-click a day): a new event / to-do on that day."""
        kind = "task" if self.kind_seg.get() == "To-do" else "event"
        return self._open_dialog(date=_date(day or self.selected), kind=kind)
    ...
    def plan(self, text=None, kind=None):
        ...
        # SPEC-AMBIGUITY: §4.15 says a leading todo/to do/task makes a to-do "otherwise an event"; v1 has a
        # kind selector instead. Both kept: the prefix wins, else the selector decides.
        m = TODO_PREFIX.match(text)
        if m and kind != "Note" and text[m.end():].strip():
            kind, text = "To-do", text[m.end():].strip()
        now = self._now()
        w = parse_typed(text, now) if text else None
    ...
    def _preview(self):
        ...
        if not self._rem_user and p["kind"] != "note":
