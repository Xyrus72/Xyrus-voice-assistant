...
Bengali script). Also: the Bangla "play" carrier words, speakable() for spoken replies and bn_acceptable() -
the sanity gate for a Bengali decode (hearing rebuild, Sep 15 2026: a faint clip decoded in Bengali comes back
as vowel-sign soup or "কে স্র স্র স্র স্র", and that must never be searched or spoken).
...
BENGALI_CONSONANT_RE = re.compile(r"[ক-হ]")       # ক..হ: a real Bengali word has one
_REPEAT_RE = re.compile(r"(.)\1{3,}")                        # the same character 4+ times in a row
BN_LP_SLACK = 0.15           # the Bengali decode may score this much worse than the English one
BN_MAX_COMPRESSION = 2.4     # len(text) / len(zlib(text)) above this = repetition, not language
BN_TOKEN_RATIO = 3           # a Bengali decode with more than 3x the English token count is babble
...
def _is_name(tok: str) -> bool:
    """The token is Xyrus's name as Whisper writes it - an EXACT member of _NAME_FORMS. It used to be a 0.72
    fuzzy match, which ate title words ("সিরিয়াস" ~ "serious")."""
    rom = transliterate(tok).strip(",.!?").lower()
    return rom in _NAME_FORMS


def has_bengali_consonant(text: str) -> bool:
    return bool(BENGALI_CONSONANT_RE.search(text or ""))


def repeated_chars(text: str) -> bool:
    """'াাাাাাাা' / 'কককক': the same character four or more times in a row - a decoder loop, not a word."""
    return bool(_REPEAT_RE.search(text or ""))


def bn_acceptable(bn_text: str | None, bn_lp: float | None, en_lp: float | None, en_tokens: int | None) -> bool:
    """Is a Bengali decode worth using (searching, speaking, showing) at all? It must contain a Bengali
    consonant, no 4x repeated character, not be Latin-only, have at most 3x the English decode's tokens, not be
    compressible past 2.4x (repetition) and score no worse than the English decode minus 0.15 avg_logprob
    (en_lp None = no constraint)."""
    text = (bn_text or "").strip()
    if not text or not has_bengali(text) or not has_bengali_consonant(text) or repeated_chars(text):
        return False
    n = len(text.split())
    if en_tokens and n > BN_TOKEN_RATIO * max(1, int(en_tokens)):
        return False
    raw = text.encode("utf-8")
    if len(raw) / max(1, len(zlib.compress(raw))) > BN_MAX_COMPRESSION:
        return False
    if bn_lp is not None and en_lp is not None and bn_lp < en_lp - BN_LP_SLACK:
        return False
    return True
...
def song_request(text: str, *, hinted: bool = False) -> str | None:
    ...
    is already known to be a song request (the English pass heard "play …", or "What should I play?") - then a
    bare Bengali title is the answer. Text without a Bengali consonant or with a 4x repeated character is never
    a request (decoder noise)."""
    if not has_bengali_consonant(text or "") or repeated_chars(text or ""):
        return None
    toks = [w.strip(",.!?।") for w in (text or "").split()]
    toks = [w for w in toks if w]
    if len(toks) >= 2 and _is_name(toks[0]):      # only the first token, only the exact name
        toks = toks[1:]
    found = hinted
    changed = True
