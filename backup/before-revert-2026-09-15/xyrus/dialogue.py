...
# What Whisper writes on breath noise ("Okay." / "Yeah." - capture.HALLUCINATIONS): never enough to confirm a
# destructive or disruptive command from the mic (hearing rebuild review, Sep 15 2026); "yes", "do it",
# "confirm", "go ahead" still are
WEAK_YES = frozenset({"okay", "ok", "yeah", "yep", "sure", "right"})
NO_WORDS = N.NO_WORDS
NOISE_MAX_WORDS = 1
MAX_ATTEMPTS = 2
...
class ConfirmDialogue(Dialogue):
    ...
    def __init__(self, ctx, question: str, on_yes: Callable[[], None], on_no: Callable[[], None] | None = None,
        ...
        self.strict = bool(getattr(cmd, "destructive", False) or getattr(cmd, "disruptive", False))
        self._q = question
        self.on_yes = on_yes
        self.on_no = on_no
    ...
    def handle(self, text: str, *, free_text: str | None = None, source: str = "mic") -> bool:
        if self.finished:
            return False
        t = " ".join(w for w in text.split() if w != "[unk]")
        if t in YES_WORDS and not (self.strict and source == "mic" and t in WEAK_YES):
            self._complete()
            return True
