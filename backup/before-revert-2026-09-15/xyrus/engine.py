...
# a destructive / disruptive confirm needs one of these (or "do it" / "go ahead"): "Yeah, yeah." / "Okay, sure."
# is what Whisper writes on breath noise (review, Sep 15 2026) - dialogue.ConfirmDialogue.strict asks again
_STRONG_YES = frozenset({"yes", "confirm", "correct", "absolutely", "definitely", "affirmative"})
# media commands that are never a song title, even while "What should I play?" is open
_MEDIA_INTERRUPT = frozenset({"pause_media", "resume_media", "play_pause", "stop_media", "next_track", "skip_track",
                              "prev_track", "volume_up", "volume_down", "volume_up_by", "volume_down_by",
                              "set_volume", "max_volume", "mute", "unmute"})
# hearing rebuild (Sep 15 2026) - the captured-command path (Whisper heard the whole utterance):
PLAY_CORRECTION_S = 90          # a bare "<title> by <artist>" this soon after a song started is a correction
PLAYBACK_S = 600                # ListenSpec.playback stays up this long after "playback: autoplay"
PLAYBACK_CLEAR = frozenset({"next_track", "skip_track", "stop_media", "play_pause", "song_not_that", "prev_track"})
POWER_NAMES = frozenset({"sleep", "hibernate", "shutdown", "restart", "lock"})
# a faint / short / doubtful power command is confirmed first: "Go to sleep" ran from a 0.5 s peak-0.03 clip
POWER_LOW_CONF, POWER_MIN_VAD_S, POWER_MIN_PEAK = -0.3, 0.8, 0.1
LOGPROB_UNSURE_DEFAULT = -0.45  # hearing.LOGPROB_UNSURE when hearing.py isn't loaded (tests, Vosk-only)
CAPTURE_ALTERNATES = 4          # n-best readings tried after Whisper's own text
# a captured request cut off at a pause ("Can you please?") - nothing to run, the user wasn't finished
_HALF_REQUEST = N.FILLER_WORDS | N.ARTICLES | frozenset({"um", "uh", "umm", "hmm", "er", "ah", "oh", "okay", "ok",
                                                         "let", "me", "please", "[unk]"})
PLAY_TITLE_Q_DEFAULT = "Play {phrase}?"
...
class Ctx:
    ...
    def __init__(self, engine: "Engine", source: str = "typed", match: Match | None = None,
        ...
        # hearing rebuild: what Whisper else heard for this utterance (songs read them with getattr)
        self.alternates: tuple[str, ...] = ()
        self.bn_text: str | None = None
...
class Engine:
    def __init__(self, *, config, registry, clock, speaker, chime, actions, executor, store, memory,
        ...
        self._captured_rearms = 0                   # "Go ahead" / "Say that again" re-arms since the last wake
        self._last_play_mono: float | None = None   # when play_song last ran (bare-title corrections)
        self._playback_until: float | None = None   # ListenSpec.playback deadline
        self._cap_tr: Transcript | None = None      # the captured transcript being routed (ctx.alternates ...)
        self._wake_cmd_tr: Transcript | None = None  # on_wake_with_command's transcript: a miss arms with one reply
        self._logprob_unsure: float | None = None
    ...
    def on_capture_timeout(self) -> None:
        """Recognizer hook (any thread; interfaces.ON_CAPTURE_TIMEOUT): the armed capture heard no speech within
        arm_timeout_s. Back to listening for the name, silently - no "Sorry" for a command that never came."""
        self.post(self._capture_timeout)

    def on_wake_with_command(self, text: str, tr: Any = None) -> None:
        """Recognizer hook (any thread; interfaces.ON_WAKE_WITH_COMMAND): Whisper heard the name AND a command in
        one breath ("Xyrus, take a screenshot"). Chime as for a wake and run it without "Yes, sir?"; when it can't
        be understood, fall back to the normal armed capture with one reply."""
        self.post(lambda: self._wake_with_command(str(text or ""), tr))
    ...
    def _ctx(self, source: str = "typed", match: Match | None = None, free_text: str | None = None,
             depth: int = 0, payload: Any = None, extra: dict | None = None) -> Ctx:
        ctx = Ctx(self, source, match, free_text, depth, payload)
        for k, v in (extra if extra is not None else self._cap_extra(self._cap_tr)).items():
            setattr(ctx, k, v)
        return ctx

    @staticmethod
    def _cap_extra(tr: Any) -> dict:
        """ctx.alternates / bn_text / confidence from a captured transcript (all optional fields: the
        capture may not carry them yet)."""
        if tr is None:
            return {}
        return {"alternates": tuple(getattr(tr, "alternates", ()) or ()), "bn_text": getattr(tr, "bn_text", None),
    ...
    def _event(self, kind: str, text: str, meta: str = "") -> None:
        ...
        if kind == "info" and text.startswith("playback: ") and text[len("playback: "):].strip() != "blocked":
            self._playback_until = self.clock.mono() + PLAYBACK_S     # commands/web autoplay check: music is on
        if kind == "noise":
    ...
    def _command_instead_of_answer(self, answer: str, free_text: str | None, *, explicit: bool) -> bool:
        ...
        d = self.dialogue
        if d is not None and getattr(d, "kind", "") == "play":
            # "What should I play?" takes any title - but a media command is not a title ("pause the song" was
            # searched and played "The Pause", Sep 15 2026): an exact one drops the question and runs
            m = self._match(answer, free_text, "full")
            return bool(m is not None and not m.has_unk and m.extra_words == 0 and m.command.name in _MEDIA_INTERRUPT)
    ...
    def _handle_captured(self, tr: Transcript) -> None:
        ...
        is already given: exactly one command runs and Xyrus is idle again; "never mind" / "cancel" / "nothing"
        end it quietly. Nothing matched -> _captured_fallthrough (hearing rebuild): one "Go ahead" / "Say that
        again" per wake, a bare "<title> by <artist>", did-you-mean, Bangla, and only then "Sorry" (no loop).
        Also accepted while "arming" (the wake reply is still being spoken)."""
        ...
        if self.dialogue is not None:
            ...
            if answer == "yes" and getattr(self.dialogue, "strict", False)                     and not (set(words) & _STRONG_YES or "do it" in text or "go ahead" in text):
                answer = "okay"                       # only weak yes words: the strict confirm asks again
        ...
        if self._quiet_cancel(text):
            ...
            return
        self._route_captured(tr, text)

    def _route_captured(self, tr: Transcript, text: str) -> None:
        """Alternates fusion: Whisper's text first, then its n-best readings; the first that parses runs (one
        from an alternate that is destructive / disruptive is only asked about). The captured text is
        full-vocabulary, so it is matched with free_text=None (free_text=text re-derived "put on shape of you"
        as the slot "on shape of you") but still handed to the command as ctx.free_text."""
        unresolved: tuple[Match, str] | None = None
        for i, h in enumerate(self._cap_hyps(tr, text)):
            m = self._match(h, None, "full")
            if m is None:
                continue
            if self._unresolved_app(m):               # "open throw": an alternate may have the real name
                if unresolved is None and i == 0:
                    unresolved = (m, h)
                continue
            if unresolved is not None and m.command.name != unresolved[0].command.name:
                continue
            if i > 0:
                self._event("info", f"heard as {h!r}")
                if m.command.destructive or m.command.disruptive:
                    self._confirm_captured(tr, m, h, R.DID_YOU_SAY)
                    return
            self._run_captured(tr, m, h)
            return
        if unresolved is not None:
            m, h = unresolved
            if not self._request_app_fix(tr, h, m):   # "open crow": re-hear the name among the known apps
                self._run_captured(tr, m, h)
            return
        self._captured_fallthrough(tr, text)

    def _cap_hyps(self, tr: Transcript, text: str) -> list[str]:
        from xyrus.capture import clean_text, strip_wake_words
        wakes = self._wake_words()
        out = [text]
        for alt in tuple(getattr(tr, "alternates", ()) or ())[:CAPTURE_ALTERNATES]:
            h = " ".join(strip_wake_words(N.normalize(clean_text(alt), wakes).split(), wakes))
            if h and h not in out:
                out.append(h)
        return out

    def _with_cap(self, tr: Transcript | None, fn: Callable[[], None]) -> None:
        """Run fn with tr as the captured transcript every Ctx made meanwhile reads (ctx.alternates ...)."""
        prev, self._cap_tr = self._cap_tr, tr
        try:
            fn()
        finally:
            self._cap_tr = prev

    def _run_captured(self, tr: Transcript, m: Match, text: str) -> None:
        if m.command.name in POWER_NAMES and self._faint(tr):
            self._event("info", f"{m.command.name} heard faintly - asking first")
            self._confirm_captured(tr, m, text, R.CONFIRM_POWER_HEARD)
            return
        self._with_cap(tr, lambda: self._run(m, "mic", text, 0))

    def _confirm_captured(self, tr: Transcript, m: Match, phrase: str, template: str) -> None:
        clean = dataclasses.replace(m, has_unk=False)
        self._pending_window = None
        self._close_window()
        ctx = self._ctx("mic", m, phrase)

        def yes() -> None:
            self._with_cap(tr, lambda: self._invoke(clean, "mic", phrase, 0))
        ctx.confirm(persona.render(template, self.config, phrase=phrase), yes)

    @staticmethod
    def _faint(tr: Transcript) -> bool:
        """A power command from a clip this doubtful is confirmed first ("Go to sleep" ran from 0.5 s at peak
        0.03). Unknown values (no confidence, vad 0 on the energy path) don't count."""
        conf = getattr(tr, "confidence", None)
        vad = float(getattr(tr, "vad_speech_s", 0.0) or 0.0)
        peak = float(getattr(tr, "peak", 0.0) or 0.0)
        return ((conf is not None and conf < POWER_LOW_CONF) or 0.0 < vad < POWER_MIN_VAD_S
                or peak < POWER_MIN_PEAK)

    def _captured_fallthrough(self, tr: Transcript, text: str) -> None:
        """Nothing parsed. In order: cut-off request -> "Go ahead"; bare "<title> by <artist>" -> a song;
        did-you-mean (registry.fuzzy_tier); Bangla (bn_text, else the language re-check); a doubtful decode
        -> "Say that again"; "Sorry"."""
        if self._filler_only(text):
            self._captured_rearm(tr, R.GO_AHEAD)
            return
        title = N.bare_title(text)
        if title and self._bare_song(tr, title, text):
            return
        tier_fn = getattr(self.registry, "fuzzy_tier", None)
        tier = tier_fn(text) if tier_fn is not None else None
        if tier is not None:
            kind, phrase, cmd = tier
            m = self._match(phrase, None, "full")
            if m is not None and m.command.name == cmd.name:
                self._event("info", f"did you mean: {text!r} ~ {phrase!r} ({kind})")
                if kind == "run" and not (cmd.destructive or cmd.disruptive):
                    self._run_captured(tr, m, phrase)
                else:
                    self._confirm_captured(tr, m, phrase, R.DID_YOU_MEAN)
                return
        bn = getattr(tr, "bn_text", None)
        if bn:
            try:
                view = dataclasses.replace(tr, text=bn, free_text=bn, language="bn")
            except TypeError:
                view = None
            if view is not None:
                self._handle_bangla(view)
                return
        if self._request_lang_check(tr):              # English Whisper may have turned Bangla into words
            return
        self._captured_miss(tr)

    @staticmethod
    def _filler_only(text: str) -> bool:
        words = text.split()
        if not words or all(w in _HALF_REQUEST for w in words):
            return True
        try:
            from xyrus.capture import only_filler
            return bool(only_filler(text))
        except Exception:
            return False

    def _recent_play(self) -> bool:
        return self._last_play_mono is not None and self.clock.mono() - self._last_play_mono < PLAY_CORRECTION_S

    def _bare_song(self, tr: Transcript, title: str, text: str) -> bool:
        """'shape of you by ed sheeran' with no "play": confirmed ("Play shape of you by ed sheeran?"), or played
        at once when a song started within PLAY_CORRECTION_S (a correction - play_song pauses the video that is
        playing before it opens the new one)."""
        cmd = self.registry.get("play_song")
        if cmd is None:
            return False
        m = Match(command=cmd, pattern=cmd.patterns[0] if cmd.patterns else None, slots={"song": title},
                  text=f"play {title}", has_unk=False, extra_words=0)
        if self._recent_play():
            self._event("info", f"song correction: {title}")
            self._with_cap(tr, lambda: self._run(m, "mic", text, 0))
        else:
            self._confirm_captured(tr, m, title, getattr(R, "PLAY_TITLE_Q", PLAY_TITLE_Q_DEFAULT))
        return True

    def _captured_miss(self, tr: Transcript) -> None:
        """Nothing understood: "Say that again" when Whisper itself was unsure (one re-arm per wake), else
        "Sorry". Inside on_wake_with_command: the normal armed capture instead, with its one wake reply."""
        if self._wake_cmd_tr is not None and self._wake_cmd_tr is tr:
            self._wake_cmd_tr = None
            self._bare_wake()
            return
        conf = getattr(tr, "confidence", None)
        if conf is not None and conf < self._logprob_unsure_value():
            self._captured_rearm(tr, R.SAY_AGAIN)
            return
        self._captured_not_heard()

    def _captured_rearm(self, tr: Transcript, reply: str) -> None:
        """One more capture ("Go ahead, sir." / "Say that again, sir?") - at most once per wake; the second miss
        is "Sorry" and idle, so there are never two Sorrys without the name."""
        if self._wake_cmd_tr is not None and self._wake_cmd_tr is tr:
            self._wake_cmd_tr = None
            self._bare_wake()
            return
        if self._captured_rearms >= 1:
            self._captured_not_heard()
            return
        self._captured_rearms += 1
        self._pending_window = None
        self._close_window()
        self.mode = "arming"
        self._say(reply)
    ...
    def _logprob_unsure_value(self) -> float:
        """hearing.LOGPROB_UNSURE when the app has loaded hearing.py (never imported here: it pulls in the
        Whisper runtime), else the literal."""
        if self._logprob_unsure is None:
            import sys
            mod = sys.modules.get("hearing") or sys.modules.get("xyrus.hearing")
            try:
                self._logprob_unsure = float(getattr(mod, "LOGPROB_UNSURE", LOGPROB_UNSURE_DEFAULT))
            except (TypeError, ValueError):
                self._logprob_unsure = LOGPROB_UNSURE_DEFAULT
        return self._logprob_unsure

    def _capture_timeout(self) -> None:
        if self.dialogue is not None or self.mode not in ("arming", "armed"):
            return
        self._pending_window = None
        self._close_window()
        self.capture_state = ""
        self._event("info", "Listening")

    def _wake_with_command(self, text: str, tr: Any) -> None:
        if self.paused:
            self._noise(text, "paused")
            return
        synth = self._synthetic_capture(text, tr)
        self._chime("wake")
        if self.dialogue is None:
            self._pending_window = None
            self._close_window()
            self.mode = "arming"
        self._captured_rearms = 0
        self._armed_misses = 0
        self._wake_cmd_tr = synth
        try:
            self._handle_captured(synth)
        finally:
            if self._wake_cmd_tr is synth and self._lang_pending is not synth:
                self._wake_cmd_tr = None              # handled (an async re-check keeps it for its callback)

    @staticmethod
    def _synthetic_capture(text: str, tr: Any) -> Transcript:
        """A CapturedTranscript of the command part, carrying the confidence fields of the recognizer's one."""
        from xyrus.capture import CapturedTranscript, clean_text
        t = clean_text(text)
        names = {f.name for f in dataclasses.fields(CapturedTranscript)}
        kw: dict[str, Any] = {}
        for name in ("raw", "confidence", "alternates", "bn_text", "vad_speech_s", "fallback_used", "words",
                     "t_start", "t_end", "song_hint"):
            if name in names and tr is not None and hasattr(tr, name):
                kw[name] = getattr(tr, name)
        peak = getattr(tr, "peak", None) if tr is not None else None
        kw.update(text=t, mode="command", source="mic", free_text=t, peak=1.0 if peak is None else float(peak),
                  captured=True, language="en")
        return CapturedTranscript(**{k: v for k, v in kw.items() if k in names})
    ...
    def _app_fix_done(self, tr: Transcript, m: Match, text: str, new_text: str) -> None:
        ...
        m2 = self._match(fixed, None, "full") if fixed else None      # full-vocabulary text: no free re-derive
        if m2 is not None and not self._unresolved_app(m2) and m2.command.name == m.command.name:
            ...
            self._run_captured(tr, m2, fixed)
        else:
            self._run_captured(tr, m, text)                # an unknown name may still be asked about (AppRef.suggest)
    ...
    def _handle_bangla(self, tr: Transcript) -> None:
        ...
        # hinted: a bare Bengali title is a song when the song question is open, the Bengali specialist ran on
        # this audio (bn_text), or a song started within PLAY_CORRECTION_S (a correction) - hearing rebuild
        hinted = (bool(getattr(tr, "song_hint", False)) or song_q or bool(getattr(tr, "bn_text", None))
                  or self._recent_play())
        title = banglish.song_request(text, hinted=hinted)
        ...
        m = _Match(command=cmd, pattern=cmd.patterns[0] if cmd.patterns else None,
                   ...
                   extra_words=0)
        self._with_cap(tr, lambda: self._run(m, "mic", title or None, 0))

    def _bare_wake(self) -> None:
        self._pending_window = None
        self._close_window()
        self._armed_misses = 0
        self._captured_rearms = 0                     # a wake reply: a fresh "Go ahead" / "Say that again"
        self.mode = "arming"
    ...
    def _run(self, m: Match, source: str, free_text: str | None, depth: int) -> None:
        ...
        if self.mode != "idle":
            self._close_window()
        ref = m.slots.get("app") if m.slots else None
        suggest = getattr(ref, "suggest", None) if ref is not None and getattr(ref, "target", "") is None else None
        if suggest:                                   # "open note hat" -> "Did you mean Notepad?" (parsing app band)
            fixed = dataclasses.replace(m, slots={**m.slots, "app": dataclasses.replace(
                ref, alias=suggest, target=getattr(ref, "suggest_target", None), suggest=None,
                suggest_target=None)})
            ctx = self._ctx(source, m, free_text, depth, payload)
            ctx.confirm(persona.render(R.DID_YOU_MEAN, self.config, phrase=suggest[:1].upper() + suggest[1:]),
                        lambda: self._invoke(fixed, source, free_text, depth, payload))
            return
    ...
    def _invoke(self, m: Match, source: str, free_text: str | None, depth: int, payload: Any = None) -> None:
        ...
        if cmd.name == "play_song":
            self._last_play_mono = self.clock.mono()
        elif cmd.name in PLAYBACK_CLEAR:
            self._playback_until = None               # skipped / stopped / paused: the music is off (or restarts)
        try:
            cmd.handler(ctx, m)
        except Exception:
    ...
    def _publish(self) -> None:
        ...
        playback = self._playback_live()
        d = self.dialogue
        with self._lock:
            if (mode, extra, playback) != (self._spec.mode, self._spec.extra_words,
                                           getattr(self._spec, "playback", False)):
    ...
    def _playback_live(self) -> bool:
        """ListenSpec.playback: from "playback: autoplay" until next / stop / pause or PLAYBACK_S."""
        until = self._playback_until
        if until is None:
            return False
        if self.clock.mono() >= until:
            self._playback_until = None
            return False
        return True
