...
def _p_app(tokens: list[str], env: SlotEnv) -> AppRef | None:
    ...
    for alias in sorted(apps, key=len, reverse=True):
        if stripped and N.phrase_in(stripped, alias.lower()):
            return AppRef(spoken, alias, apps[alias])
    import difflib                              # v1 match_app: 'the calculater' -> calculator
    close = difflib.get_close_matches(stripped or spoken, [a.lower() for a in apps], n=1, cutoff=0.75)
    if close:
        alias = next(a for a in apps if a.lower() == close[0])
        return AppRef(spoken, alias, apps[alias])
