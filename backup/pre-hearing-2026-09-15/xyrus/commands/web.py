...
SECTION = "Apps & web"
LOOKUP_TIMEOUT = 8.0          # youtube_search's own urlopen timeout is 6 s (§3.6); +2 s for the oEmbed fallback
...
def _pause_other_audio(actions) -> bool:
    ...
    probe = getattr(actions, "browser_playing", None)
...
def play_song(ctx, title: str) -> None:
    ...
    def found(res):
        _LAST[ctx.engine] = LastSong(query=title, tried=[res[1]])
        _start_found(ctx, res)
    ...
    ctx.do(a.find_song, title, then=found, error=failed, timeout=LOOKUP_TIMEOUT)
...
def next_song(ctx) -> None:
    ...
    ctx.do(find, st.query, then=got, error=failed, timeout=LOOKUP_TIMEOUT)
